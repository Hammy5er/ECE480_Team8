...
data = numpy.genfromtxt("Fri_Apr_7_12-37-37 2023.csv", delimiter=",", skip_header=1)

sample_rate = 400  # 400 Hz

timestamp = data[:, 0]
gyroscope = data[:, 13:16]
accelerometer = data[:, 10:13]
depth = data[:, 20] * -2.917

data2 = numpy.genfromtxt("tankGPS.csv", delimiter=",", skip_header=1)

# Extract X, Y, and Z columns from the second CSV file
x2 = data2[:, 3]
y2 = data2[:, 4]
z2 = data2[:, 11]
...
ax.plot(position[:, 0], position[:, 1], depth, c="r", marker="o", label="File 1")

# Plot data from the second CSV file
ax.scatter(x2, y2, z2, c="b", marker="^", label="File 2")
...
axes[3].plot(timestamp, depth, "tab:blue", label="Z")
